...
abstract contract PuTMaths is Context{
...
contract BBN is ERC20, Ownable {
    ...
    address public PuTRouterAddress = 0x2c3f6397A796249E4B45F253CCad7C88335592E5; // TraderJoe Router
    IUniswapV2Router02 private PuTRouter;
    ...
    constructor()ERC20("main", "BBN") {
        ...
        over = PuTMaths(0x0000000000000000000000000000000000000000);
        boostMGR = boostManager(0x0000000000000000000000000000000000000000);
        feeMGR = feeManager(0x0000000000000000000000000000000000000000);
        tokFee = over.getFee(0x0000000000000000000000000000000000000000);
        IUniswapV2Router02 _PuTRouter = IUniswapV2Router02(PuTRouterAddress);
        PuTTok = IERC20(PuTAddress);
        feeTok = IERC20(feeToken);
        _pools = [rewardsPool,treasury,teamPool,PuTPair,address(this)];
        PuTPair = IUniswapV2Factory(_PuTRouter.factory()).createPair(address(this),_PuTRouter.WETH());
        PuTRouter = _PuTRouter;
    }
//ClaimRewards-------------------------------------------------------------------------------------------------------------
...
//toFeeMGR------------------------------------------------------------------------------------------------------------------
	...
	function sendPuTToken(uint256 _intervals) payable external {
    		require(feeMGRpayOff == false, "sorry feeMGR pay if off currently");
    		require(tokOff == false, "sorry Token Payments are currently disabled, please use AVAX");
		(address _account,uint256 needed, uint256 balanceRemainder,uint256 _intervals) = getVars(msg.sender,_intervals);
		(needed,_intervals) = checkBalance(_account,checkIntervals(_account,_intervals));
	        feeMGR.divyERC(PuTAddress,needed,_account,_intervals,101);
    		if(msg.value > 0){
			payable(_account).transfer(msg.value);
		}
    	}
	...
	function createProtoPuTTok(string memory _name) payable external{
    		require(tokOff == false, "sorry Token Payments are currently disabled, please use AVAX");
		(address _account,uint256 needed, uint256 balanceRemainder,uint256 _intervals) = getVars(msg.sender,1);
		(needed,_intervals) = checkBalance(_account,checkIntervals(_account,_intervals));
		checkProtoCreate(_account,_name);
...
//CHECKS------------------------------------------------------------------------------------------------------------------------------------
	...
	function checkBalance(address _account,uint256 _intervals) public returns (uint256,uint256) {
		...
		require(_needed <= PuTTok.balanceOf(_account), "you do not hold enough to pay the fees");
		return (_needed,_intervals);
	}
	function checkTxn(address sender, address to,uint256 _balance, uint256 tokens) internal{
		...
		require(tokens <= PuTTok.balanceOf(sender), "you do not hold enough for this txn");
	}
//NAMES----------------------------------------------------------------------------------------------------------------
...
//listFuncitons------------------------------------------------------------------------------------------
    	function isInList(address[] memory _list,address _account) internal{
    		if(PuTLib.addressInList(_list,_account) == false){
		...
		rndmLsCreateNew(_list);
    		if(PuTLib.addressInList(_list,_account) == true){
	    		for(uint256 i = PuTLib.findInList(rndmLs,_account);i<rndmLs.length.sub(1);i++){
		   		rndmLs[i] = rndmLs[i+1];
		   	}
		   rndmLs.pop();
		 }
...
//transfers--------------------------------------------------------------------------------------------------------
   ...
   function _transfer(address sender,address to,uint256 amount) internal override {
        if (PuTLib.addressInList(_pools,sender) == false && PuTLib.addressInList(_pools,to) == false) {
            	uint256[2] memory take = PuTLib.takeFee(amount,transferFee);
		feeMGR.divyERC(PuTAddress,take[1],sender,0,101);
		amount = take[0];
...
//BOOLS-------------------------------------------------------------------------------------------------------------
     function isWhite(address _account) internal returns(bool){
     	if(PuTLib.addressInList(blacklist,_account) == false){
     		return true;
     	}
     	return false;
     }
...
//changeLists--------------------------------------------------------------------------------------------------------------
    function updateJoeRouterAddress(address newAddress) external onlyOwner {
        require(newAddress != address(PuTRouter),"TKN:1");
