...
contract PuTProtoStarDrops is Ownable{
	using SafeMath for uint256;
	struct DROPS{
		uint256 dropped;
		uint256 claimed;
...
//getNUMS----------------------------------------------------------------------------------------------------------
	function createForAlldropped(uint _x, uint _y,uint256[] memory strucNum) external onlyOwner{
		_y = PuTLib.getLower(_y,uint(2467));
		address add;
		for(uint i=_x;i<_y;i++){
			add = dropMGR2.getclaims(i);
			if(PuTLib.addressInList(gDropped,add) ==false){
	...
	function makegetclaimed(uint _x, uint _y) external onlyOwner(){
		...
		for(uint j = _x;j<_y;j++){
			...
			if(PuTLib.addressInList(gClaims,add) ==false){
				gClaims.push(add);
			}
		}
	}
	function getallElse(uint _x, uint _y) external onlyOwner()  {
		_y = PuTLib.getLower(_y,gClaims.length);
		uint _k;
		uint i;
		uint j;
		address _account;
		for(j= _x;j < _y ; j++ ){
			_account = gClaims[j];
			if(PuTLib.addressInList(gElse,_account) ==false){
