...
abstract contract oracle is Context {
	 function getFee() external virtual view returns(uint256);
}
contract PuTFeeManager is Ownable{
	using SafeMath for uint256;
	struct TOTALFEES{
...
//PercentageSplits-----------------------------------------------------------------------------------------------------------------------------
	function payFeeAvax(address _account,uint256 _intervals, uint256 _x) payable external onlyGuard{
	    ...
	    payable(teamPool).transfer(PuTLib.sendPercentage(_amount,teamPerc));
	    payable(rewardsPool).transfer(PuTLib.sendPercentage(_amount,rewardsPerc));
	    payable(treasury).transfer(PuTLib.sendPercentage(_amount,treasuryPerc));
	    uint256 balanceRemainder = _amount.sub(PuTLib.sendPercentage(_amount,treasuryPerc)).sub(PuTLib.sendPercentage(_amount,rewardsPerc)).sub(PuTLib.sendPercentage(_amount,teamPerc));
	...
	function divyERC(address token, uint256 _amount,address _account, uint256 _intervals, uint256 _x) payable external onlyGuard() {
	    ...
	    Tok.transferFrom(_account, teamPool, PuTLib.sendPercentage(_amount,teamPerc));
	    Tok.transferFrom(_account, rewardsPool, PuTLib.sendPercentage(_amount,rewardsPerc));
	    Tok.transferFrom(_account, treasury, PuTLib.sendPercentage(_amount,treasuryPerc));
	    uint256 leftover =  _amount.sub(PuTLib.sendPercentage(_amount,teamPerc)).sub(PuTLib.sendPercentage(_amount,rewardsPerc)).sub(PuTLib.sendPercentage(_amount,treasuryPerc));
...
//FeesQuery-------------------------------------------------------------------------------------------------
	function INTqueryProtoFutureFees(address _account,uint256 _x) internal {
		protoowners[_account][_x].protoFutureFees = PuTLib.getMultiple(feePeriod,INTgetProtoDueElapsed(_account,_x));
	}
	function INTqueryProtoTotalPayable(address _account,uint256 _x) internal {
		protoowners[_account][_x].protoTotalPayable = PuTLib.getMultiple(feePeriod,PuTLib.getLower(INTgetProtoCollapseDate(_account,_x).sub(block.timestamp),maxFeePayment)).sub(protoowners[_account][_x].protoFutureFees);
	}
...
//TimeUpdates-------------------------------------------------------------------------------------------------
	...
	function INTupdateProtoFeeFroze(address _account,uint256 _x,uint256 _time) internal {
		if(feeFroze == false && INTgetFeeFroze() != Zero){
			protoowners[_account][_x].protoFeeFroze =_time.sub(PuTLib.getHigher(INTgetFeeFroze(),INTgetProtoCreationDate(_account,_x)));
...
//TimeQuery-------------------------------------------------------------------------------------------------
	...
	function INTqueryProtoDueElapsed(address _account,uint256 _x) internal{
		...
		if(PuTLib.isLower(next,block.timestamp)==true){
...
//onlyOwnerFunctions----------------------------------------------------------------------------------------------------------------------------------
	...
	function removeManagers() external onlyOwner() {
    	...
    	function changeFeePeriod(uint256 _days) external onlyOwner() {
    		uint256 maxPeriods = PuTLib.getMultiple(feePeriod,maxFeePayment);
    		feePeriod = _days.mul(1 days);
    		maxFeePayment = maxPeriods.mul(feePeriod);
        }
        function changePuTTokenAddress(address _account) external onlyOwner(){
        	PuTToken = _account;
        	PuT = IERC20(PuTToken);
        }
