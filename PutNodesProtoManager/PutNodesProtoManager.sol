...
abstract PutNodesUpdateManager is Context{
...
abstract contract prevProtoStarManager is Context {
...
contract PutNodesProtoManager is Ownable {
    string public constant name = "NebulaProtoManager";
    ...
    PutNodesUpdateManager public updateMGR;
    address public nftAddress;
    address payable public treasury;
    modifier managerOnly() {require(PutNodesLib.addressInList(Managers,msg.sender)== true || _msgSender() == owner()); _;}
...
//nameProtos----------------------------------------------------------------------------------------------------------------------------
   function nameExists(address _account, string memory _name) internal view returns(bool){
    ...
    function changeName(address _account, string memory _name,string memory new_name) external {
    	...
    	require(PutNodesLib.addressInList(PROTOaccounts,_account) == true,"you do not hold any Protostars Currently");
    	PROTOstars[] storage protos = protostars[_account];
...
//CreateStars-----------------------------------------------------------------------------------------------------------------------------------
   ...
   function INTaddProto(address _account,string memory _name) internal{
   	...
   	require(nameExists(_account,_name) == false,"name has already been used");
       	if (PutNodesLib.addressInList(PROTOaccounts,_account) == false){
...
//getStarsData-----------------------------------------------------------------------------------------------------------
    function protoAccountExists(address _account) external returns (bool) {
    	return PutNodesLib.addressInList(PROTOaccounts,_account);
    }
...
//deadStars--------------------------------------------------------------------------------------------------------------
	...
	boostMGR.collapseProto(_account,proto.name);
    	...
    	if (PutNodesLib.addressInList(DeadStars,_account) == false){
...
//changeWallets-----------------------------------------------------------------------------------------------
    function updatePutNodesToken(address _account) external onlyGuard(){
    	INTupdatePutNodesToken(_account);
    }
    function INTupdatePutNodesToken(address _account) internal{
    	PutNodesToken = _account;
    	PutNodesTok = IERC20(PutNodesToken);
    }
