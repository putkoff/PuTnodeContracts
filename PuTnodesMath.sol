...
library SafeMath {
    ...
    function tryMul(uint256 a, uint256 b) internal pure returns (bool, uint256) {
        unchecked {
            ...
            // bePuTt is lost if 'b' is also tested.
  ...
  Authorizable
{
  ...
  uint feeDec = 6;
  uint RegDec = 18;
  uint256 public fee = PuTLib.getDeces(feeNum,feeDec);
  uint256[3] public nftCost = [PuTLib.getDeces(300,RegDec),PuTLib.getDeces(750,RegDec),PuTLib.getDeces(1500,RegDec)];
  ...
  constructor(address[] memory swaps,uint256[] memory _boosts,uint256[3] memory _nftCost,uint256[] memory _vars,uint256[] memory _fees,uint256[] memory _cashoutRed){
	  ...
	  for(uint i=0;i<3;i++){
	  	nftCost[i] = PuTLib.getDeces(_nftCost[i],RegDec);
	  }
	  ...
	  for(uint i=0;i<4;i++){
	  	...
	  	nftCashoutPercs[i] = PuTLib.getDeces(_cashoutRed[i],17);
	  }
  }
  function getStablePrice() public view returns(uint256){
    ...
    return PuTLib.getDeces(_stable.div(_main.div(PuTLib.get256(_decMain))),10);
   }
 ...
 function doPrecision(uint256 _x,uint256 perc) external view returns(uint256,uint256,uint256){
 	return PuTLib.doPrecision(_x,perc);
 }
 ...
 function getFee() external view returns(uint256) {
 	 uint256 _fee = PuTLib.getDeces(fee,RegDec).div(getStablePrice()*100);
   	 return PuTLib.make18Dec(_fee,PuTLib.getDecimals(_fee)-feeDec);
   	 
   }
 function getModFee(uint256 _val) external view returns(uint256) {
 	 uint256 _fee = PuTLib.getDeces(_val,RegDec).div(getStablePrice()*100);
   	 return PuTLib.make18Dec(_fee,PuTLib.getDecimals(_fee)-PuTLib.getLower(PuTLib.getDecimals(_fee),feeDec));
 }
 function getNftPrice(uint _val) external view returns(uint256) {
   	 return PuTLib.getDeces(nftCost[_val],RegDec).div(getStablePrice()*100);
 }
 ...
 function updateNftCost(uint256[3] memory _vals) external onlyOwner() {
 	for(uint i=0;i<3;i++){
	  	nftCost[i] = PuTLib.getDeces(_vals[i],RegDec);
	  }
 }
 function feeDecQuery() internal {
 	fee = PuTLib.getDeces(feeNum,feeDec);
 }
