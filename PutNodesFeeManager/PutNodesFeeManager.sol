...
contract PutNodesFeeManager is Ownable{
	using SafeMath for uint256;
	struct TOTALFEES{
	...
	address public PutNodesToken;
	address public PutNodesPair;
	uint256 teamPerc;
	...
	IERC20 feeTok;
	IERC20 PutNodes;
	modifier onlyManager() {require(_msgSender() == owner() || PutLib.addressInList(Managers,_msgSender()), "NOT_Manager");_;}
...
//PercentageSplits-----------------------------------------------------------------------------------------------------------------------------
	function payFeeAvax(address _account,uint256 _intervals, uint256 _x) payable external onlyGuard{
	    ...
	    payable(teamPool).transfer(PutNodesLib.sendPercentage(_amount,teamPerc));
	    payable(rewardsPool).transfer(PutNodesLib.sendPercentage(_amount,rewardsPerc));
	    payable(treasury).transfer(PutNodesLib.sendPercentage(_amount,treasuryPerc));
	    uint256 balanceRemainder = _amount.sub(PutNodesLib.sendPercentage(_amount,treasuryPerc)).sub(PutNodesLib.sendPercentage(_amount,rewardsPerc)).sub(PutNodesLib.sendPercentage(_amount,teamPerc));
	...
	function divyERC(address token, uint256 _amount,address _account, uint256 _intervals, uint256 _x) payable external onlyGuard() {
	    ...
	    Tok.transferFrom(_account, teamPool, PutNodesLib.sendPercentage(_amount,teamPerc));
	    Tok.transferFrom(_account, rewardsPool, PutNodesLib.sendPercentage(_amount,rewardsPerc));
	    Tok.transferFrom(_account, treasury, PutNodesLib.sendPercentage(_amount,treasuryPerc));
	    uint256 leftover =  _amount.sub(PutNodesLib.sendPercentage(_amount,teamPerc)).sub(PutNodesLib.sendPercentage(_amount,rewardsPerc)).sub(PutNodesLib.sendPercentage(_amount,treasuryPerc));
...
//FeesQuery-------------------------------------------------------------------------------------------------
	function INTqueryProtoFutureFees(address _account,uint256 _x) internal {
		protoowners[_account][_x].protoFutureFees = PutNodesLib.getMultiple(feePeriod,INTgetProtoDueElapsed(_account,_x));
	}
	function INTqueryProtoTotalPayable(address _account,uint256 _x) internal {
		protoowners[_account][_x].protoTotalPayable = PutNodesLib.getMultiple(feePeriod,PutNodesLib.getLower(INTgetProtoCollapseDate(_account,_x).sub(block.timestamp),maxFeePayment)).sub(protoowners[_account][_x].protoFutureFees);
	}
...
//TimeUpdates-------------------------------------------------------------------------------------------------
	...
	function INTupdateProtoFeeFroze(address _account,uint256 _x,uint256 _time) internal {
		if(feeFroze == false && INTgetFeeFroze() != Zero){
			protoowners[_account][_x].protoFeeFroze =_time.sub(PutNodesLib.getHigher(INTgetFeeFroze(),INTgetProtoCreationDate(_account,_x)));
...
//TimeQuery-------------------------------------------------------------------------------------------------
	...
	function INTqueryProtoDueElapsed(address _account,uint256 _x) internal{
		...
		if(PutNodesLib.isLower(next,block.timestamp)==true){
...
//onlyOwnerFunctions----------------------------------------------------------------------------------------------------------------------------------
	...
	function removeManagers() external onlyOwner() {
    	...
    	function changeFeePeriod(uint256 _days) external onlyOwner() {
    		uint256 maxPeriods = PutNodesLib.getMultiple(feePeriod,maxFeePayment);
    		feePeriod = _days.mul(1 days);
    		maxFeePayment = maxPeriods.mul(feePeriod);
        }
        function changePutNodesTokenAddress(address _account) external onlyOwner(){
        	PutNodesToken = _account;
        	PutNodes = IERC20(PutNodesToken);
        }
