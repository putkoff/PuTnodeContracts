...
contract FeeManager is Ownable{
	...
	function payFeeTokenSpec(address _account, uint256 _intervals, uint256 _x) payable external onlyGuard(){
	    require(PutLib.addressInList(protoOwners,_account) == true,"you are not in the protolist"); 
	    require(getProtoOwnersLength(_account)<_x,"you do not have that many Protos");
	    require(maxPayPeriods > (PutLib.safeDiv(feePeriod,INTgetProtoDueElapsed(_account,_x))).add(PutLib.doMultiple(_intervals,feePeriod)),"you have paid more fees than you are able to for this proto"); 
	    uint256 total = PutLib.doMultiple(_intervals,tokFee);
	...
	function checkAvaxSend(address _account,uint256 _sent,uint256 _intervals) internal returns (uint256,uint256,uint256){
		require(PutLib.addressInList(protoOwners,_account) == true,"you are not in the protolist"); 
		(uint256 unusedIntervals,uint256 _intervals,uint256 newFee,uint256 balanceReturn) = PutLib.doAllMultiple(_intervals,_sent,over.getFee());
	...
	function INTaddProto(address _account,string memory _name) internal{
		...
		_protoowners.push(PROTOOWNERS({
			...
			protoFeeFroze:Zero,
			protoTotalPayable:PutLib.doMultiple(maxFeePayment.sub(feePeriod),feePeriod)
		}));		
	...
	function INTupdateProtoTotalPayable(address _account,uint256 _x) internal {
		...
		uint256 newTot = PutLib.getLower(maxLife,maxFeePayment).div(feePeriod);
		protoowners[_account][_x].protoTotalPayable = newTot;
		if(PutLib.isEqual(tot,newTot) == false){
			if(PutLib.isLower(tot,newTot) == true){
	...
	function INTupdateBoolCollapsed(address _account,uint256 _x) internal {
		...
		if(PutLib.isLower(INTgetProtoCollapseDate(_account,_x),block.timestamp)==true){
