...
contract PuTnodesBoostManager is Ownable {
    string public constant name = "PuTnodesBoostManager";
    ...
    constructor() {
	    over = overseer(0x0000000000000000000000000000000000000000);
	    NFTMGR = NFTStake(0x0000000000000000000000000000000000000000);
	    for(uint i = 0;i<4;i++){
...
//updateNums--------------------------------------------------------------------------------------------------
    ...
    function updateNums() internal{
    	for(uint i=0;i<4;i++){
    		RewardsPerSec[i] = RewardsPerSec[i].add(PuTLib.sendPercentage(RewardsPerSec[i],RewardsPercentage[i]));
    	}
    }
//changeWallets-----------------------------------------------------------------------------------------------
